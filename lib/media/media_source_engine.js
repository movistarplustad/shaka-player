...
shaka.media.MediaSourceEngine = class {
  ...
  async appendBuffer(
    ...
    if (contentType == ContentType.TEXT) {
      ...
      await this.textEngine_.appendBuffer(
          ...
          // Current playback position, so the parser can show the cues near
          // the playhead first (window-first parse) on a large single-file
          // TTML.  See TtmlTextParser.parseMediaChunked.
          this.video_ ? this.video_.currentTime : null);
      return;
    }
  ...
  async setStreamProperties(
    ...
    // On WebOS 3 (Chrome 38) and WebOS 4 (Chrome 53), the MSE implementation
    // uses '<=' instead of '<' when checking samples against appendWindowStart,
    // dropping samples whose presentation timestamp is exactly 0.0 when
    // appendWindowStart=0. This affects start-over streams whose first segment
    // begins at presentation time 0. We cannot set appendWindowStart to a
    // negative value because Chrome 38/53 rejects it (throws, causing error
    // 3015). Instead we nudge timestampOffset by +1ms so that samples which
    // would land at exactly 0.0 arrive at 0.001s, safely above the window.
    if (!ignoreTimestampOffset &&
        appendWindowStart === 0 &&
        timestampOffset !== 0 &&
        (shaka.util.Platform.isWebOS3() || shaka.util.Platform.isWebOS4())) {
      timestampOffset += 0.001;
      console.log('Nudging timestampOffset by +1ms to work around WebOS MSE');
    }
