...
shaka.media.DrmEngine = class {
  ...
  constructor(playerInterface, updateExpirationTime = 1) {
    ...
    this.licenceRequiredCallback_ = null;
  ...
  newInitData(initDataType, initData) {
    ...
    if (!shaka.util.Platform.isTizen2()) {
      ...
      let force = false;
      // If force is true then req specifies whether we need
      // a licence request.  Otherwise check for existing initData
      let req;
      if (this.licenceRequiredCallback_) {
        // Need to find the initData for current keySystem
        const defaultSystemIds_ = new Map()
        ...
        const systemIds = defaultSystemIds_;
        ...
        if (pssh) {
          ...
          if (index >= 0) {
            ...
            [force, req] = this.licenceRequiredCallback_(ksInitData, keyStat);
          }
        }
      }
  ...
  /**
   * Sets a callback used to determine whether a new licence is required
   * based on initData and keyStatuses
   * @param {function(*,*)|null} callback
   */
  setLicenceRequiredCallback(callback) {
    this.licenceRequiredCallback_ = callback;
  }
