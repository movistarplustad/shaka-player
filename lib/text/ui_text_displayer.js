...
shaka.text.UITextDisplayer = class {
   ...
   */
  append(cues) {
    // [DIAG TEMPORAL] medir dedup (Cue.equal recursivo) vs updateCaptions.
    const __t0 = Date.now();
    ...
    const __dedupMs = Date.now() - __t0;
    const __t1 = Date.now();
    this.updateCaptions_();
    const __updMs = Date.now() - __t1;
    if (__dedupMs > 50 || __updMs > 50) {
      console.warn('[TTML diag] append: dedup=' + __dedupMs + 'ms' +
          ' updateCaptions=' + __updMs + 'ms cuesList=' + cuesList.length +
   ...
   * @param {{visited: number}} stats [DIAG] contador de nodos visitados
   * @private
   */
  updateCuesRecursive_(cues, container, currentTime, parents, stats) {
    ...
    for (const cue of cues) {
      ...
      if (cue.nestedCues.length > 0 && wrapper) {
        this.updateCuesRecursive_(
            cue.nestedCues, wrapper, currentTime, parents, stats);
      }
  ...
  updateCaptions_(forceUpdate = false) {
    ...
    if (this.isTextVisible_) {
      ...
      // Update the cues.
      // [DIAG TEMPORAL] medir coste del walk/creacion de DOM.
      const __t0 = Date.now();
      const __sizeBefore = this.currentCuesMap_.size;
      const __stats = {visited: 0};
      this.updateCuesRecursive_(
          this.cues_, this.textContainer_, currentTime, /* parents= */ [],
          __stats);
      const __ms = Date.now() - __t0;
      if (__ms > 50) {
        console.warn('[TTML diag] updateCaptions=' + __ms + 'ms visited=' +
            __stats.visited + ' created=' +
            (this.currentCuesMap_.size - __sizeBefore) +
            ' totalCuesMap=' + this.currentCuesMap_.size);
      }
