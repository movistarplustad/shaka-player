...
shaka.text.TextEngine = class {
   ...
   */
  destroy() {
    // Clear any parser-level persistent cache. TtmlTextParser keeps a static
    // parse cache so re-selecting a track is instant, but it must not outlive
    // the player (memory). Bracket access: only TtmlTextParser implements it,
    // and it is @export'd so ADVANCED mode does not rename it.
    const parser = /** @type {?} */ (this.parser_);
    if (parser && parser['clearParseCache']) {
   ...
   * @param {?string=} uri
   * @param {?number=} currentTime current playback position, used by the
   *   chunked TTML parser to parse and show the cues near the playhead first.
   * @return {!Promise}
   */
  async appendBuffer(buffer, startTime, endTime, uri, currentTime) {
    goog.asserts.assert(
    ...
    // Parse the buffer and add the new cues.  TtmlTextParser exposes a
    // non-blocking parseMediaChunked() that yields to the event loop while
    // parsing, so the UI does not freeze on a large single-file TTML on slow
    // devices (e.g. webOS 2016).  Other parsers only have synchronous
    // parseMedia(), so we fall back to it.
    const parser = /** @type {?} */ (this.parser_);
    const data = shaka.util.BufferUtils.toUint8(buffer);
    let allCues;
    // [DIAG TEMPORAL] medir ruta + tiempos parse/append.
    const __t0 = Date.now();
    // Window-first: did the parser emit a window of cues near the playhead
    // already?  If so, the full append below must keep those cues visible
    // (phase 2 loads the rest in the background without hiding the track).
    let emittedWindow = false;
    // Bracket access so Closure ADVANCED does not rename this optional,
    // exported method (only TtmlTextParser implements it).
    if (parser['parseMediaChunked']) {
      console.warn('[TTML diag] CHUNKED path');
      // Called by the parser with the cues near currentTime, before the full
      // parse finishes, so a subtitle shows in ~1-2s.  Append + show them now;
      // the rest streams in afterwards (phase 2) keeping these visible.
      const onPartial = (windowCues) => {
        if (!this.parser_ || !this.displayer_ || this.parser_ !== parser) {
          return;
        }
        const filtered = windowCues.filter((cue) => {
        ...
        emittedWindow = true;
        // Franja temporal que cubren los primeros subtitulos (el cue contenedor
        // de la ventana: start = min de los anidados, end = max).
        let minS = Infinity;
        let maxE = -Infinity;
        ...
        console.warn('[TTML diag] >>> PRIMEROS SUBS disponibles a T+' +
            (Date.now() - __t0) + 'ms | cubren [' + minS.toFixed(1) + 's, ' +
            maxE.toFixed(1) + 's] | playhead=' +
            (currentTime != null ? currentTime.toFixed(1) : '?') + 's | ' +
            filtered.length + ' top cue(s)');
        this.displayer_.append(filtered, /* keepVisible= */ true);
      };
      allCues = await parser['parseMediaChunked'](
          data, time, uri, currentTime, onPartial);
      // The chunked parse yields to the event loop, so by now the engine may
      // have been destroyed, or the text stream may have been switched (which
      // replaces the parser via initParser).  In either case these cues are
      // stale; drop them so we don't render the wrong track's subtitles.
      if (!this.parser_ || !this.displayer_ || this.parser_ !== parser) {
        console.warn('[TTML diag] dropped stale parse');
        return;
      }
    } else {
      console.warn('[TTML diag] SYNC path (parseMediaChunked NOT found!)');
      allCues = parser.parseMedia(data, time, uri);
    }
    const __t1 = Date.now();
    ...
    // If a window was already shown, keep it visible while this (phase 2)
    // append loads the rest in the background.  Otherwise keep the original
    // behavior (the chunked append hides the track while adding to avoid
    // freezing the video on weak devices).
    this.displayer_.append(cuesToAppend, emittedWindow);
    const __t2 = Date.now();
    // PARSEO COMPLETO terminado a T+__t1. El append del resto (fase 2) corre en
    // segundo plano; su fin se ve en la traza "RESTO cargado" del displayer.
    console.warn('[TTML diag] <<< PARSEO COMPLETO a T+' + (__t1 - __t0) +
        'ms | topCues=' + allCues.length + ' | append(resto) lanzado en ' +
        (__t2 - __t1) + 'ms (corre en background)');
