...
shaka.text.TtmlTextParser = class {
   ...
   * @export
   */
  async parseMediaChunked(data, time, uri) {
  ...
  static yieldToEventLoop_() {
    return new Promise((resolve) => {
      ...
      // conformance rule).  We sleep a real amount (not 0ms) so that on weak
      // devices (webOS 2016) the video decode/compositor pipeline gets a
      // contiguous window to do work between our parse chunks.  A bare
      // setTimeout(0) yields the JS thread but the gaps are too short for the
      // decoder to keep up, so the video appears frozen even though JS yields.
      (new shaka.util.Timer(resolve)).tickAfter(
          shaka.text.TtmlTextParser.YIELD_SLEEP_MS_ / 1000);
    });
  }
   ...
   * @param {boolean} isContent
   * @param {?(string|undefined)} uri
   * @param {{next: number, count: number, lastYield: number, maxGap: number}}
   *   deadline shared, mutable wall-clock yield deadline (next), yield counter
   *   and diagnostics (lastYield, maxGap)
