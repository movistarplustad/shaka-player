...
/**
 * PERF: sleep (ms) por cesion una vez superada la rampa. Con el parseo
 * "ventana primero" los subtitulos ya se muestran casi al instante, asi que el
 * parseo completo NO tiene urgencia: usamos un sleep generoso (igual que la
 * rampa) para no robarle CPU al decodificador de video y que el buffer no baje.
 * Antes era 8ms (~65% CPU al parseo) y agotaba el buffer, provocando un ~1s de
 * congelacion al terminar. Subir = mas suave pero parseo mas lento.
 *
 * @const {number}
 * @private
 */
shaka.text.TtmlTextParser.WARM_SLEEP_MS_ = 30;
 ...
 * @private
 */
shaka.text.TtmlTextParser.WINDOW_FWD_S_ = 90;
