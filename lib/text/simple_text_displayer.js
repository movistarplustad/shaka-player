...
shaka.text.SimpleTextDisplayer = class {
  ...
  append(cues) {
    ...
    if (SimpleTextDisplayer.countLeafCues_(cues) >
      ...
      this.appendCuesChunked_(cues, generation);
      return;
    }
   ...
   * @private
   */
  async appendCuesChunked_(cues, generation) {
    const SimpleTextDisplayer = shaka.text.SimpleTextDisplayer;
    const Utils = shaka.text.Utils;
    ...
    const pairs = [];
    SimpleTextDisplayer.collectLeafCues_(cues, null, pairs);
    ...
    // Snapshot the existing native cues once for de-duplication (the
    // synchronous path also snapshots once).
    const existing = this.textTrack_.cues ?
        Array.from(this.textTrack_.cues) : [];
    ...
    // webOS. Add all the cues with the track temporarily "hidden" (no render),
    // then restore the intended visibility once at the end.
    ...
    for (const pair of pairs) {
      ...
      for (const inCue of Utils.getCuesToFlatten([pair.cue], pair.parent)) {
        const duplicate = existing.some((c) => {
          return c.startTime == inCue.startTime &&
              c.endTime == inCue.endTime && c.text == inCue.payload;
        });
        if (duplicate) {
          continue;
        }
        const vttCue = SimpleTextDisplayer.convertToTextTrackCue_(inCue);
        if (vttCue) {
      ...
      if (Date.now() - batchStart >= SimpleTextDisplayer.CHUNK_BUDGET_MS_) {
        // eslint-disable-next-line no-await-in-loop
        await SimpleTextDisplayer.yieldToEventLoop_();
        batchStart = Date.now();
      }
    }

    // Restore the intended visibility now that all cues are in place. If a
    // newer append/remove took over (generation changed), it owns the mode now.
    if (this.textTrack_ && this.appendGeneration_ == generation) {
      this.textTrack_.mode = this.textVisible_ ? 'showing' : 'hidden';
    }
    console.warn('[TTML diag] chunked append DONE (subs visible now) in ' +
        (Date.now() - __t0) + 'ms, added=' + __added +
        ', visible=' + this.textVisible_);
  }
 ...
 * @private
 */
shaka.text.SimpleTextDisplayer.CHUNK_BUDGET_MS_ = 30;
 ...
 * @private
 */
shaka.text.SimpleTextDisplayer.CHUNK_SLEEP_MS_ = 15;
