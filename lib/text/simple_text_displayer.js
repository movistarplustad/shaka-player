...
shaka.text.SimpleTextDisplayer = class {
  ...
  async appendCuesChunked_(cues, generation, keepVisible) {
    ...
    // For the window-first background load (keepVisible) the subtitles are
    // already on screen, so there is no urgency: add the cues in smaller
    // batches separated by longer sleeps so the video decoder never starves
    // and the video does not freeze. The normal (hidden) path keeps the
    // original, faster cadence.
    const budgetMs = keepVisibleEffective ?
        shaka.text.SimpleTextDisplayer.BG_CHUNK_BUDGET_MS_ :
        shaka.text.SimpleTextDisplayer.CHUNK_BUDGET_MS_;
    const sleepMs = keepVisibleEffective ?
        shaka.text.SimpleTextDisplayer.BG_CHUNK_SLEEP_MS_ :
        shaka.text.SimpleTextDisplayer.CHUNK_SLEEP_MS_;

    // Yield once before starting, so the (synchronous) end-of-parse flush and
    // the cue collection/sort above don't chain straight into the first batch:
    // give the decoder a contiguous window first.
    if (keepVisibleEffective) {
      // eslint-disable-next-line no-await-in-loop
      await shaka.text.SimpleTextDisplayer.yieldToEventLoop_(sleepMs);
    }
   ...
   *
   * @param {number=} sleepMs sleep duration; defaults to CHUNK_SLEEP_MS_.
   * @return {!Promise}
   * @private
   */
  static yieldToEventLoop_(sleepMs) {
    const ms = sleepMs != null ? sleepMs :
        shaka.text.SimpleTextDisplayer.CHUNK_SLEEP_MS_;
    return new Promise((resolve) => {
      (new shaka.util.Timer(resolve)).tickAfter(ms / 1000);
    });
  }
...
/**
 * PERF (window-first): budget (ms) of synchronous addCue work between yields
 * for the background (keepVisible) append. Smaller than CHUNK_BUDGET_MS_ so
 * each add-while-showing burst is shorter and never freezes the video. The
 * subtitles are already visible, so a slower background load is fine.
 *
 * @const {number}
 * @private
 */
shaka.text.SimpleTextDisplayer.BG_CHUNK_BUDGET_MS_ = 8;

/**
 * PERF (window-first): real sleep (ms) on each yield of the background
 * (keepVisible) append. Longer than CHUNK_SLEEP_MS_ so the decoder always has
 * room. Trades a slower background load for zero freezing.
 *
 * @const {number}
 * @private
 */
shaka.text.SimpleTextDisplayer.BG_CHUNK_SLEEP_MS_ = 100;
